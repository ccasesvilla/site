...
def Unpublished_List(request):
    ...
    return render(request, 'blog/unpublished.html', {'page_obj': page_obj})
...
def edit_blog(request, entry_id):
    ...
    if request.method != 'POST':
        ...
        if postForm.is_valid():
            postForm.updated_on = datetime.datetime.now()  
            postForm.save()
...
def update(request):
    if request.method == 'POST':
        repo = git.Repo("ccvCoding.pythonanywhere.com ")
        origin = repo.remotes.origin

        origin.pull()

        return HttpResponse("Updated code 2 PythonAnywhere")

    else:
        return HttpResponse("Couldn't update the code 2 PythonAnywhere")